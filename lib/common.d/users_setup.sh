...
function setup_user()
{
    ...
    if [[ -f $SNOW_HOME/${user_name}/.ssh/id_rsa ]]; then
        ...
        error_msg "Error setting up sNow! SSH keys."
    fi
} 1>>$LOGFILE 2>&1
...
function setup_env()
{
    ...
    ln -sf $SNOW_ROOT/bin/snow-source.sh /etc/profile.d/snow.sh
    ln -sf $SNOW_ROOT/bin/snow-source.csh /etc/profile.d/snow.csh
} 1>>$LOGFILE 2>&1
